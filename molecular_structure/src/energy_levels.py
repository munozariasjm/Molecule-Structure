...
class MoleculeLevels(object):
    ...
    def eigensystem(self,Ez_val,Bz_val,method='torch',order=True, set_attr=True, normalize=False,disable_trap=False,angle=None, chop=None):
        ...
        if self.trap and not disable_trap:
            evals,evecs = self.la_tool.diagonalize(self.H_function(Ez_val,Bz_val,self.I_trap,self.theta_trap),order=order, normalize=normalize,round=self.round)
        elif self.trap and disable_trap:
            evals,evecs = self.la_tool.diagonalize(self.H_function(Ez_val,Bz_val,self.I_trap,self.theta_trap),order=order, normalize=normalize,round=self.round)
        else:
            evals,evecs = self.la_tool.diagonalize(self.H_function(Ez_val,Bz_val),order=order, normalize=normalize,round=self.round)
        if chop is not None:
    ...
    def AngleMap(self,angle_array, Ez_val, Bz_val, I_trap = None,output=False,write_attribute=True,method='torch',initial_evecs=None,**kwargs):
        ...
        evals_t,evecs_t = self.la_tool.diagonalize_batch(angle_matrices,round=self.round)
        # t2 = perf_counter()
...
#Is this redundant?
    ...
    def trap_shift_EB(self,I_trap=None,theta_trap=None,Ez=None,Bz=None,step=0.1):
        ...
        if theta_trap is None:
            theta_trap = self.theta_trap
        evals,evecs = self.la_tool.diagonalize(self.H_function(Ez,Bz,I_trap,theta_trap),round=self.round)
    ...
    def trap_shift_evecs(self,evals,evecs,I0,theta,Ez,Bz,step=0.1):
        evals0,evecs0 = evals,evecs
        evals1,evecs1 = self.la_tool.diagonalize(self.H_function(Ez,Bz,I0*(1-step),theta),round=self.round)
        order = self.la_tool.state_ordering(evecs0,evecs1,round=self.round)
        # evecs1_ordered = evecs1[order,:]
        evals1_ordered = evals1[order]
        # sgn = np.sign(np.diag(evecs1_ordered@evecs0.T))
    ...
    def EB_grid(self,Ez,Bz,E_or_B_first = 'E',reverse=False,interp=False,method='torch',output=False,evecs=False,PTV=False,trap_shifts=False,order_states=True,EDM_or_MQM='EDM'):
        ...
        if reverse:
            ...
            Bz = Bz[::-1]
        if E_or_B_first == 'E':
            evals_E,evecs_E = self.StarkMap(Ez,Bz[0],output=True,write_attribute=False,order=order_states)
            for i in range(N_Ez):
                evals_B, evecs_B = self.ZeemanMap(Bz, Ez_val = Ez[i],output=True,write_attribute=False,initial_evecs=evecs_E[i],order=order_states)
                for j in range(N_Bz):
            ...
            evals_B,evecs_B = self.ZeemanMap(Bz,Ez[0],output=True,write_attribute=False,order=order_states)
            for i in range(N_Bz):
                evals_E, evecs_E = self.StarkMap(Ez, Bz_val = Bz[i], output=True,write_attribute=False,initial_evecs = evecs_B[i],order=order_states)
                for j in range(N_Ez):
